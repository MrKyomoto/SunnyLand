...
namespace engine::resource
{
    ...
    AudioManager::AudioManager()
    {
        if (!MIX_Init())
        {
            throw std::runtime_error("AudioManager 构造失败: MIX_Init 失败" + string(SDL_GetError()));
        }
        ...
        if (!mixer_)
        {
            ...
            throw std::runtime_error("AudioManager 构造失败: Mixer指针为空" + string(SDL_GetError()));
        }
    ...
    MIX_Audio *AudioManager::loadLongAudio(const string &file_path)
    {
        ...
        long_audios_.emplace(file_path, std::unique_ptr<MIX_Audio, SDLMixAudioDeleter>(longAudio));
    ...
    MIX_Audio *AudioManager::loadShortAudio(const string &file_path)
    {
        ...
        short_audios_.emplace(file_path, std::unique_ptr<MIX_Audio, SDLMixAudioDeleter>(shortAudio));
