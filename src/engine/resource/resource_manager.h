...
public:
  ...
  explicit ResourceManager(
      SDL_Renderer *
          renderer); // explicit
                     // 关机案子用于防止隐式转换,对于单一参数的构造函数,通常考虑添加
  ~ResourceManager(); // 显式声明析构函数,为了能让智能指针正确管理仅有前向声明的类,
                      // 老师的原话是如果这个地方不显式声明,那么编译器在编译阶段会内联一个析构函数,
                      // 而由于我们这里是前向声明子类,所以内联的结果是不对的,实际上我们在texture_manager.cpp里
                      // #include了真正的子类,虽然在cpp里只是 ~() =
                      // default但就已经存在真正的子类了
