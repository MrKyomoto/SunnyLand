...
bool Config::loadFromFile(const string &file_path) {
  ...
  if (!file.is_open()) {
    spdlog::warn("配置文件 '{}'未找到, 使用默认设置并创建默认配置文件",
                 file_path);
  ...
  try {
    ...
    spdlog::error("读取配置文件 '{}' 时出错: {}, 使用默认设置", file_path,
                  e.what());
  }
  return false;
}
bool Config::saveToFile(const string &file_path) {
  ...
  try {
    ...
    file << j.dump(4);
    spdlog::info("成功将配置保存到 '{}'", file_path);
    return true;
  } catch (const std::exception &e) {
...
void Config::fromJson(const nlohmann::json &j) {
  ...
  if (j.contains("input_mappings") && j["input_mappings"].is_object()) {
    ...
    try {
      auto input_mappings =
          mappings_json.get<std::unordered_map<string, vector<string>>>();

      input_mappings_ = std::move(input_mappings);
      spdlog::trace("成功从配置加载输入映射");
    } catch (const std::exception &e) {
      spdlog::warn("配置加载警告: 解析 'input_mappings' "
                   "时发生异常,使用默认映射, 错误: {}",
                   e.what());
    }
  } else {
    spdlog::trace(
        "配置跟踪: "
        "未找到'input_mappings'部分或不是对象,使用头文件中定义的默认映射");
  }
}
