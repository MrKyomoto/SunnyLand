...
bool GameApp::initSDL() {
  ...
  window_ = SDL_CreateWindow("SunnyLand", config_->window_width_,
  ...
  // 设置 VSync
  // (开启时,驱动程序会尝试将帧率限制到显示器刷新率,有可能会覆盖我们手动设置的target_fps)
  int vsync_mode = config_->vsync_enabled_ ? SDL_RENDERER_VSYNC_ADAPTIVE
                                           : SDL_RENDERER_VSYNC_DISABLED;
  SDL_SetRenderVSync(sdl_renderer_, vsync_mode);
  spdlog::trace("VSync 设置为: {}",
                config_->vsync_enabled_ ? "Enabled" : "Disabled");

  // 设置逻辑分辨率,视口大小应该与这个一致(针对像素游戏,逻辑分辨率设置为窗口大小的一半,
  // 设成一半的原因是内部只渲染1/4的像素量,计算量小,同时每个原始像素对应4个屏幕像素,像素边缘锐利,不会出现半像素模糊,锯齿发虚等问题)
...
void GameApp::testGameObject() {
  ...
  game_object.addComponent<engine::component::SpriteComponent>(
      "assets/textures/Props/big-crate.png", resource_manager_.get(),
      engine::utils::Alignment::CENTER);
  game_object.getComponent<engine::component::TransformComponent>()->setScale(
      glm::vec2(2.0f, 2.0f));
