...
Camera::Camera(const glm::vec2 &viewport_size, const glm::vec2 &position,
    ...
    : viewport_size_(viewport_size), position_(position),
...
glm::vec2
Camera::worldToScreenWithParallax(const glm::vec2 &world_pos,
...
void Camera::clampPosition() {
  // 边界检查需要确保相机视图(position 到 position + viewport
  // size)在limit_bounds内
  if (limit_bounds_.has_value() && limit_bounds_->size.x > 0 &&
    ...
    // 确保 max_cam_pos >= min_cam_pos (视口可能比世界大)
