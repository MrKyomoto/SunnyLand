...
namespace engine::object
{
    ...
    public:
        ...
        {
            // 检测组件是否合法, static_assert(condition,message):静态断言,在编译期检测,无任何性能影响, std::is_base_of<Base,Derived>::value 判断Base类型是否是Derived类型的基类
            static_assert(std::is_base_of<engine::component::Component, T>::value, "T 必须继承自 Component");

            // 获取类型标识, typeid(T)用于获取一个表达式或类型的运行时类型信息(RTTI),返回std::type_info&
            // std::type_index 针对std::type_info对象的包装器,主要设计用来作为关联容器的键
            ...
            // std::forward用于实现完美转发,传递多个参数的时候使用...标识, 和前边Args&&... args配合使用,&时是左值引用,&&时就是右值引用, 简单来说用这个是效率最高的
            auto new_component = std::make_unique<T>(std::forward<Args>(args)...);
            T *ptr = new_component.get(); // 先获取裸指针以便于返回
            new_component->setOwner(this);
            components_[type_index] = std::move(new_component); // 移动组件,此时new_component为空不可再使用
